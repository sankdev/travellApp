...
const volController = {
  ...
  getAll: async (req, res) => {
    try {  
      const vols = await Vol.findAll({
        ...
        { model: Agency, as: 'volAgency' } // Ajout de l'inclusion de l'agence
      ],
    });
  ...
  getAllByAgency: async (req, res) => {
    try {
      const userId = req.user.id; // Supposons que req.user contient l'utilisateur connecté après l'authentification
  
      // Vérifier si l'utilisateur appartient à une agence
      const agency = await Agency.findOne({ where: { userId } });
  
      if (!agency) {
        return res.status(403).json({ message: "Vous n'avez pas accès aux vols d'agence." });
      }
  
      // Récupérer uniquement les vols liés à cette agence
      const vols = await Vol.findAll({
