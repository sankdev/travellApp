...
const FlightAgency = () => {
    ...
    const fetchVols = async () => {
        try {
            const response = await volService.getVols();
             console.log('volAgence',response) 
    ...
    return (
        <div className="max-w-5xl mx-auto p-6">
            <h1 className="text-3xl font-bold text-orange-800 mb-6">Manage Flight Agencies</h1>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div>
                    ...
                    >
                        ...
                        {vols.map((vol) => (
                            <option key={vol.id} value={vol.id}>{vol.name}-{vol.companyVol.name}</option>
                        ))}
                    </select>
                </div>
            ...
            <button
                        type="submit"
                        disabled={loading}
                        className={`w-md px-4 py-2 text-white bg-orange-600 hover:bg-orange-700 rounded-md shadow-sm focus:ring-2 focus:ring-offset-2 focus:ring-orange-400 ${
            ...
            <div className="overflow-x-auto">
                <table className="table-auto w-full bg-white shadow-lg rounded-lg overflow-hidden">
                    <thead className="bg-gray-200 text-orange-700">
                        <tr>
                            <th>Vol</th><th>Company</th><th>Depart</th><th>Arrival</th><th>Agency</th><th>Price</th><th>Departure Time</th><th>Arrival Time</th><th>Actions</th>
                        </tr>
                    </thead>
                    <tbody>
                        {flightAgencies.map((fa) => (
                            <tr key={fa.id} className="border-b hover:bg-gray-100">
                                <td>{fa.flight?.name || "N/A"}</td>
                                 <td>{fa.flight?.companyVol.name || "N/A"}</td>
                                  <td>{fa.flight?.origin?.name || "N/A"}</td>
                                  <td>{fa.flight?.destination?.name || "N/A"}</td>                  
