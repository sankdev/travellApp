...
import { paymentModeService } from '../../services/paymentModeService'; // Service existant
...
const InvoicePayment = () => {
    ...
    const loadData = async () => {
        ...
        try {
            ...
            const agencyId = invoiceData.agencyId || invoiceData.agency?.id;
            
            if (agencyId) {
                // 3. Charger les informations de l'agence
                try {
                    const agencyResponse = await agencyService. getInvoicesForAgency();
                   console.log('agenceInvoice',agencyResponse.data)
                ...
                // Fallback: charger tous les modes si pas d'agence
                await fetchAllPaymentModes();
            }
    ...
    // Nouvelle fonction pour récupérer les modes par agence
    const fetchPaymentModesByAgency = async (agencyId) => {
        try {
            ...
            if (response.data && Array.isArray(response.data)) {
                ...
                console.warn("Format de données inattendu, fallback vers tous les modes");
                await fetchAllPaymentModes();
            }
        } catch (err) {
            console.error("Erreur chargement modes par agence:", err);
            // Fallback: charger tous les modes en cas d'erreur
            await fetchAllPaymentModes();
        }
    };

    // Fallback: tous les modes actifs
    const fetchAllPaymentModes = async () => {
        try {
             const response = await paymentModeService.getPaymentModesByAgency(agencyId);
              console.log('modePay',response)
               setPaymentModes(response.data || []);
        } catch (err) {
            console.error("Erreur chargement tous les modes:", err);
            setPaymentModes([]);
        }
    };
    ...
    const fetchPaymentHistory = async () => {
        try {
            const response = await paymentService.getPaymentsByInvoice(invoiceId);
    ...
    const handlePayment = async (e) => {
        ...
        // Validation supplémentaire selon le mode
        if (selectedModeDetails?.requiresReference && !paymentReference) {
            setError(`Veuillez saisir la référence de paiement pour ${selectedModeDetails.name}.`);
            return;
        }
    ...
    const confirmPayment = async () => {
        ...
        try {
            const paymentData = {
                invoiceId,
                paymentModeId: parseInt(selectedMode),
    ...
    const getPaymentModeIcon = (modeName) => {
        const icons = {
            'Carte Bancaire': faCreditCard,
            'Virement': faUniversity,
            'Mobile Money': faMobile,
            'Espèces': faWallet,
            'Chèque': faReceipt,
            'PayPal': faPaypal,
            'Visa': faCcVisa,
        ...
        return icons[modeName] || faMoneyBillWave;
    };
                    ...
                    <div className="lg:col-span-1 space-y-6">
                        ...
                        {agency && (
                            <div className="bg-white rounded-2xl shadow-sm p-6 border-l-4 border-indigo-500">
                                ...
                                {agency.location && (
                                    <p className="text-sm text-gray-600 mt-1">{agency.location}</p>
                                )}
                        ...
                        <div className="bg-white rounded-2xl shadow-sm p-6">
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="bg-gray-50 rounded-xl p-4">
                                    ...
                                    <p className="text-2xl font-bold text-gray-900">{formatAmount(invoice.total)}</p>
                            ...
                            <div className="bg-white rounded-2xl shadow-sm p-6">
                                ...
                                <div className="space-y-3">
                                    {paymentHistory.map((payment, index) => {
                                        const mode = paymentModes.find(m => m.id === payment.paymentModeId);
                                        return (
                                            <div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                                                <div className="flex items-center">
                                                    <div className="w-8 h-8 rounded-full bg-indigo-100 flex items-center justify-center mr-3">
                                                        <FontAwesomeIcon 
                                                            icon={getPaymentModeIcon(mode?.name)} 
                                ...
                                <form onSubmit={handlePayment} className="space-y-6">
                                            ...
                                            {paymentModes.map((mode) => (
                                                <button
                                                    ...
                                                    className={`
                                                        ...
                                                        ${!mode.isActive ? 'opacity-50 cursor-not-allowed' : ''}
                                                    `}
                                                ...
                                                >
                                                    ...
                                                    `}>
                                                        <FontAwesomeIcon 
                                                            icon={getPaymentModeIcon(mode.paymentMode?.organisation || mode.name)}
                                                            className={selectedMode === mode.id.toString() ? 'text-white' : 'text-gray-600'}
                                                        />
                                                    </div>
                                                    <div className="text-left">
                                                        <p className="font-medium text-gray-900">
                                                            {mode.paymentMode?.type || mode.name}
                                                        </p>
                                                        {mode.commission && (
                                                            <p className="text-xs text-gray-500">
                                                                Commission: {mode.accountNumber}%
                                                            </p>
                                                        )}
                                                    </div>
                                                </button>
                                    ...
                                    {selectedModeDetails && (
                                        <div className="space-y-4 animate-fadeIn">
                                            ...
                                            {selectedModeDetails.requiresReference && (
                                                <div>
                                                    <label className="block text-sm font-medium text-gray-700 mb-2">
                                                    ...
                                                    <input
                                                        ...
                                                        placeholder={`Ex: ${selectedModeDetails.referenceExample || 'Numéro de transaction'}`}
                                                        ...
                                                        required={selectedModeDetails.requiresReference}
                                                    />
                                                    <p className="mt-1 text-xs text-gray-500">
                                                        {selectedModeDetails.referenceHelp || "Ce numéro servira de référence pour votre paiement"}
                                                    </p>
                                                </div>
                                            )}

                                            {/* Instructions spécifiques */}
                                            {selectedModeDetails.instructions && (
                    ...
                    <div className="bg-white rounded-2xl max-w-md w-full p-6">
                        ...
                        <div className="space-y-4 mb-6">
                            ...
                            <div className="flex justify-between items-center">
                                ...
                                <span className="font-medium">{selectedModeDetails?.paymentMode?.name || selectedModeDetails?.name}</span>
                            </div>
