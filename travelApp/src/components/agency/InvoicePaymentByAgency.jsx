...
const InvoicePayment = () => {
    ...
    // ✅ ÉTAPE 1 : État pour suivre si la facture est payée
    ...
    // ✅ ÉTAPE 2 : Mise à jour des détails du mode sélectionné
    useEffect(() => {
        if (selectedMode) {
    ...
    const loadData = async () => {
        ...
        try {
            ...
            const paid = invoiceData.status === 'paid' || invoiceData.balance <= 0;
            setIsInvoicePaid(paid);
            console.log(`📊 Statut facture: ${paid ? 'PAYÉE' : 'En attente'}`);
    ...
    // ✅ ÉTAPE 3 : Fonction pour récupérer les modes par agence avec sélection auto
    const fetchPaymentModesByAgency = async (agencyId) => {
        try {
            ...
            console.log(`✅ ${modes.length} modes de paiement chargés`);
            
            // ✅ Sélectionner automatiquement le mode par défaut
            if (modes.length > 0) {
                const defaultMode = modes.find(m => m.isDefault === true);
                
                if (defaultMode) {
                    // Si un mode par défaut existe, le sélectionner
                    console.log(`🎯 Mode par défaut trouvé:`, defaultMode);
                    setSelectedMode(defaultMode.id.toString());
                } else {
                    // Sinon, sélectionner le premier mode disponible
                    console.log(`🎯 Aucun mode par défaut, sélection du premier mode:`, modes[0]);
                    setSelectedMode(modes[0].id.toString());
                }
            }
    ...
    const confirmPayment = async () => {
        ...
        try {
            const paymentData = {
                ...
                modePaymentId: parseInt(selectedMode),
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="lg:col-span-2">
                        <div className="bg-white rounded-2xl shadow-sm p-6">
                            ...
                            {/* ✅ AFFICHAGE CONDITIONNEL - SI FACTURE PAYÉE */}
                            {isInvoicePaid ? (
                                <div className="text-center py-8 bg-green-50 rounded-xl border-2 border-green-200">
                                    ...
                                    <h3 className="text-xl font-bold text-gray-900 mb-2">Facture entièrement payée</h3>
                                    <p className="text-gray-600 mb-4">
                                        Cette facture a été réglée. Aucun paiement supplémentaire n'est nécessaire.
                                    </p>
                                    <div className="inline-block bg-green-100 text-green-800 px-4 py-2 rounded-lg font-medium">
                                        Montant total payé : {formatAmount(invoice.amount)}
                                    </div>
                                    
                                    {/* Bouton pour voir l'historique */}
                                    <div className="mt-6">
                                        <button
                                            onClick={() => {/* Scroll vers historique */}}
                                ...
                                <form onSubmit={handlePayment} className="space-y-6">
                                    ...
                                    <div className="mb-4 p-3 bg-blue-50 rounded-lg flex items-center gap-2">
                                        ...
                                        <span className="text-sm text-blue-700">
                                            Mode de paiement par défaut pré-sélectionné
                                        </span>
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                            {paymentModes.map((mode) => {
                                                const isSelected = selectedMode === mode.id.toString();
                                                const isDefault = mode.isDefault === true;
                                                ...
                                                return (
                                                    <button
                                                        ...
                                                        className={`
                                                            p-4 border-2 rounded-xl flex items-center transition-all relative
                                                            ${isSelected
...
// Ajout des styles pour l'animation (à placer dans votre fichier CSS global)
