...
const PricingRuleCRUD = () => {
  ...
  const fetchData = async () => {
   ...
   console.log('userFlightAgency',vols)
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold text-center mb-6  text-orange-800">Manage Pricing Rules</h1>
        ...
        >
          ...
          {vols.map((vol) => (
            <option key={vol.id} value={vol.id}>
              {vol.flight.name}-{vol.flight.companyVol.name}
            </option>
          ))}
        </select>
      ...
      <button
        type="submit"
        disabled={loading}
        className={`w-md mt-4 px-4 py-2 text-white font-semibold bg-orange-600 rounded-md
 shadow-md transition duration-200 ease-in-out transform hover:scale-105 hover:bg-orange-700 focus:ring-2 focus:ring-offset-2 focus:ring-orange-400 ${
      ...
      >
       ...
       <h2 className="text-xl font-semibold mb-4 text-orange-700">Pricing Rule List</h2>
             ...
             {pricingRules.map((rule) => (
               <tr key={rule.id} className="text-center">
                 ...
                 <td className="border p-2 flex justify-center space-x-4">
                   <FontAwesomeIcon icon={faEdit} className="text-orange-600 cursor-pointer" onClick={() => handleEdit(rule)} />
                   <FontAwesomeIcon icon={faTrash} className="text-orange-600 cursor-pointer" onClick={() => handleDelete(rule.id)} />
                 </td>
               </tr>
             ))}
           </tbody>
