...
const ReservationDetailByAgency = () => {
                        ...
                        <thead className="bg-gray-50">
                            <tr>
                                ...
                                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Document Type</th>
                                 <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Docume Number</th>
  ...
  {reservation?.passengers?.map((passenger, index) => (
    <tr key={index}>
      ...
      {/* Liste des documents du passager */}
        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
  {passenger.document?.length > 0 || passenger.documents?.length > 0 ? (
    <ul className="space-y-2">
      {/* Afficher les documents depuis passenger.document */}
      {passenger.document?.map((doc, docIndex) => (
        <li key={`doc-${docIndex}`} className="flex flex-col space-y-1">
          <div>
            <span className="font-medium">{doc.documentType}:</span> {doc.documentNumber}
          </div>
          {doc.files?.map((file, fileIndex) => (
            <a
              key={`file-${fileIndex}`}
              href={file.base64}
              download={`document_${docIndex}_${fileIndex}.pdf`}
              className="text-blue-500 hover:underline flex items-center"
            >
              <svg className="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
              </svg>
              Télécharger {file.name || 'document'}
            </a>
          ))}
        </li>
      ))}
      
      {/* Afficher les documents depuis passenger.documents */}
      {passenger.documents?.map((doc, docIndex) => {
        const apiBaseUrl = import.meta.env.VITE_API_BASE_URL || 'http://localhost:5000';
        const baseUrl = apiBaseUrl.replace('/api', '');
        const documentUrl = doc.documentPath?.startsWith("http")
          ? doc.documentPath
          : `${baseUrl}/${doc.documentPath?.replace(/\\/g, '/')}`;

        return (
          <li key={`db-doc-${docIndex}`} className="flex items-center space-x-2">
    ...
    <span className="text-gray-400">Aucun document</span>
     ...
     {/* Date d'émission */}
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                              {passenger.document?.length > 0 ? (
                                <ul>
                                  {passenger.document.map((doc, docIndex) => (
                                    <li key={docIndex}>{doc.documentNummber }</li>
                                  ))}
                                </ul>
                              ) : (
                            ...
                            {/* Date d'expiration */}
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
  {passenger.document?.[0]?.issueDate || passenger.documents?.[0]?.issueDate ? (
    new Date(passenger.document?.[0]?.issueDate || passenger.documents?.[0]?.issueDate).toLocaleDateString('fr-FR')
  ) : (
    <span className="text-gray-400">N/A</span>
  )}
</td>
...
<td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
  {passenger.document?.[0]?.expirationDate || passenger.documents?.[0]?.expirationDate ? (
    new Date(passenger.document?.[0]?.expirationDate || passenger.documents?.[0]?.expirationDate).toLocaleDateString('fr-FR')
  ) : (
    <span className="text-gray-400">N/A</span>
  )}
</td>
                    ...
                    <dl className="divide-y divide-gray-200">
                        ...
                        {reservation?.campaign && (
                            <div className="px-6 py-5">
                                ...
                                <dd className="mt-1 text-sm text-gray-900">{reservation.campaign.title}</dd>
                                <dd className="mt-1 text-sm text-gray-900">{reservation.campaign.price}</dd>
                            </div>
                        )}
                    </dl>
                </div>
