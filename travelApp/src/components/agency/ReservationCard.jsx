...
const ReservationList = () => {
  ...
  const handleConfirm = async (id) => {
    setProcessingId(id);
    try {
      await reservationService.combineConfirmReservation(id);
...
const ReservationCard = ({ reservation, onStatusChange, isProcessing, getDestinationById }) => {
  const shouldShowConfirmButton = ['pending', 'proposal_accepted'].includes(
      ...
      <div className="mt-4 flex justify-between items-center">
        {shouldShowConfirmButton && (
          <ConfirmButton 
            ...
            handleConfirm={() => onStatusChange(reservation.id)} 
          />
        )}
        <Link
...
const Header = ({ reservation }) => {
  ...
  const statusColors = {
    ...
    Cancelled: 'bg-red-100 text-red-800',
      proposal_accepted: 'bg-blue-100 text-blue-800',
  };
