...
const ClassAgency = () => {
    ...
    const fetchUserAgency = useCallback(async () => {
        ...
        try {
            ...
            // Si response est directement un objet
    ...
    const fetchFlightAgenciesByAgency = useCallback(async (agencyId) => {
        try {
            ...
            const allFlightAgencies = response?.data || response || [];
            
            // Filtrer pour ne garder que ceux de l'agence connectée
    ...
    const fetchClassAgenciesByAgency = useCallback(async (agencyId, agencyFlightIds) => {
        try {
            ...
            const allClassAgencies = response?.data || response || [];
            
            // Filtrer pour ne garder que celles dont le vol appartient à l'agence
            const agencyClasses = allClassAgencies.filter(classItem => 
                agencyFlightIds.includes(classItem.agencyVolId)
            );
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            // 1️⃣ Récupérer l'agence de l'utilisateur
            ...
            // 2️⃣ Récupérer les classes (toujours les mêmes pour tous)
            const classesRes = await classeService.getClasses();
            const classesData = classesRes?.data || classesRes || [];
            setClasses(Array.isArray(classesData) ? classesData : []);

            // 3️⃣ Récupérer les FlightAgency de cette agence
            ...
            // 4️⃣ Récupérer les ClassAgency de cette agence
    ...
    const handleSubmit = async (e) => {
        ...
        // Vérifier que le vol appartient bien à l'agence de l'utilisateur
    ...
    return (
        <div className="min-h-screen bg-gray-50 py-4 sm:py-8 px-4 sm:px-6 lg:px-8">
            <div className="max-w-7xl mx-auto">
                                ...
                                >
                                    ...
                                    {flightAgencies.map((fa) => (
                                        <option key={fa.id} value={fa.id}>
                                            {fa.flight?.name} - {fa.flight?.origin?.city} → {fa.flight?.destination?.city}
                                        </option>
                                    ))}
                                </select>
                                {flightAgencies.length === 0 && (
                ...
                {/* Liste des associations - Version responsive avec tableau scrollable */}
                                                    ...
                                                    <div className="flex justify-between items-start">
                                                        <div>
                                                            <p className="font-medium text-gray-900">
                                                                {item.class?.name || 'Classe inconnue'}
                                                            </p>
