...
const initialFormState = {
    ...
    gender: '', profession: '', birthDate: '', birthPlace: '', nationality: ''
};
a
const CustomerProfile = () => {
    ...
    const fetchProfiles = async () => {
        ...
        try {
            ...
            // setTotalPages(response.meta?.totalPages || 1); // À ajouter si pagination
        } catch (err) {
    ...
    try {
        if (editMode) {
            await customerService.updateCustomerProfile({ ...formData, userId: editId });
    ...
    return (
        <div className="container mx-auto px-4 py-8">
            ...
            {message.text && (
                <div className={`text-${message.type === 'error' ? 'red' : 'green'}-500 mb-4 text-center`}>
                    {message.text}
                </div>
            )}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {Object.keys(initialFormState).map((key) => (
                        <div key={key}>
                            ...
                            <input
                                type={key === 'birthDate' ? 'date' : key.includes('email') ? 'email' : 'text'}
                                name={key}
                                value={formData[key]}
            ...
            <div className="mb-4">
                <input
                    ...
                    className="w-full h-8 px-2 py-1 border rounded-md text-xs"
                />
            </div>
