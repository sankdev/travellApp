            ...
            try {
                const [profileRes, reservationsRes, invoicesRes] = await Promise.allSettled([
                    ...
                    customerService.getCustomerInvoices()
                     ...
                     console.log("invoicesRes.value.data:", invoicesRes?.value?.data);
                ...
                if (isMounted.current) {
                    setDashboardData({
                        ...
                        invoices: invoicesRes.status === "fulfilled" && Array.isArray(invoicesRes.value.data.data) ? invoicesRes.value.data.data : []
                    });
...
// }, []);
    ...
    const activeReservations = dashboardData.reservations.filter(res => res.status === 'confirmed');
                                ...
                                {dashboardData.reservations.slice(0, 5).map((reservation) => (
                                    <tr key={reservation.id}>
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            {reservation.startDestination?.city}-{reservation.endDestination?.city}
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap">
