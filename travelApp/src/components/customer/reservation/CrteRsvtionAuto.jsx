...
const CreateReservation = () => {
  ...
  const loadDataFromClassAgencyId = async (classAgencyId) => {
    try {
      ...
      // 1️⃣ Récupérer les détails de la ClassAgency
  ...
  const populateFormWithClassData = async (classAgencyId) => {
    try {
      ...
      const updatedFormData = {
        ...
        startDestinationId: volData?.origin?.id || '',
        endDestinationId: volData?.destination?.id || '',
        agencyClassId: classData?.id || '',
        companyId: volData?.companyVol?.id || '',
        destinationId: volData?.destination?.id || '',
        tripType: 'one-way',
        campaignId: '',
        returnVolId: '',
      ...
      // Charger toutes les classes disponibles pour ce vol
      if (flightData?.id) {
  ...
  const loadClassesForFlight = async (agencyVolId, selectedClassId = null) => {
    try {
      ...
      if (Array.isArray(classesData) && classesData.length > 0) {
        ...
        // Si un ID de classe est fourni, le sélectionner
        if (selectedClassId) {
...
const calculateTotalPrice = async () => {
  try {
    ...
    for (const passenger of passengers) {
      ...
      if (passengerType === 'ADLT') {
        ...
        // CORRECTION: Chercher la règle uniquement avec agencyClassId
        // Les types de passagers dans pricingRules sont "ADL", "CHD", "INF"
        ...
        if (rule) {
          ...
          console.log(`${passengerType === 'CHD' ? '🧒 Enfant' : '🍼 Bébé'}: +${rulePrice} (règle)`);
        } else {
          totalPrice += adultBasePrice;
          console.log(`${passengerType}: +${adultBasePrice} (prix adulte)`);
        }
      }
    }
    ...
    if (formData.tripType === 'round-trip' && formData.returnVolId) {
      ...
      if (returnClass) {
        const returnPrice = parseFloat(returnClass.price) || 0;
        
        // Calculer le prix retour pour chaque passager
        for (const passenger of passengers) {
          const passengerType = passenger.typePassenger || 'ADLT';
          
          if (passengerType === 'ADLT') {
            totalPrice += returnPrice;
            console.log(`🔄 Retour adulte: +${returnPrice}`);
          } else {
            const rulePassengerType = passengerType === 'ADLT' ? 'ADL' : passengerType;
            ...
            if (returnRule) {
              ...
              console.log(`🔄 Retour ${passengerType}: +${returnRule.price} (règle)`);
            } else {
              totalPrice += returnPrice;
              console.log(`🔄 Retour ${passengerType}: +${returnPrice} (prix adulte)`);
            }
          }
        }
      } else {
        // Fallback: même prix que l'aller
        totalPrice += adultBasePrice * passengers.length;
        console.log(`🔄 Retour (fallback): +${adultBasePrice * passengers.length}`);
      }
    }
    ...
    try {
      ...
      for (const passenger of passengers) {
        ...
        if (passengerType === 'ADLT') {
          ...
          const rule = pricingRules.find(r => 
            r.agencyVolId === selectedClass.agencyVolId &&
            r.agencyClassId === selectedClass.id &&
      ...
      if (formData.tripType === 'round-trip' && formData.returnVolId) {
        ...
        if (returnClass) {
          const returnPrice = parseFloat(returnClass.price) || 0;
          totalPrice += returnPrice * passengers.length;
          console.log(`🔄 Retour: +${returnPrice * passengers.length}`);
        } else {
          totalPrice += adultBasePrice * passengers.length;
          console.log(`🔄 Retour (fallback): +${adultBasePrice * passengers.length}`);
        }
    ...
    setPassengers(prev =>
      prev.map((passenger, i) => {
        if (i === index) {
          ...
          if (key === 'typePassenger') setTimeout(() => calculateTotalPrice(), 100);
          return updated;
        }
        return passenger;
      })
  ...
  useEffect(() => {
    calculateTotalPrice();
  }, [formData.agencyClassId, passengers, classes, formData.tripType]);
                      ...
                      return (
                        <div
                          ...
                          onClick={() => {
                            ...
                            setTimeout(() => calculateTotalPrice(), 10s);
                          }}
                          className={`
                        ...
                        >
                          ...
                          {cls.priceMultiplier && cls.priceMultiplier !== 1 && (
                          ...
                          <p className="text-sm">
                            ...
                            {passenger.typePassenger === 'ADLT' ? (
                              ...
                              (() => {
                                ...
                                const price = rule ? parseFloat(rule.price) : 
                                    (classes.find(c => c.id === parseInt(formData.agencyClassId))?.price || 0);
                                return (
                                  <span className="text-green-600 font-bold">
