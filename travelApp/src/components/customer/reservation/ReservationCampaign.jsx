...
// ============================================
const FlightInfoDisplay = ({ campaign }) => {
...
const ManualFlightSelection = ({
 ...
 // États pour le filtrage des vols
...
const [agencyReturnVolCount, setAgencyReturnVolCount] = useState(0);  
  // MODE DE RECHERCHE PAR COMPAGNIE
...
const toggleReturnFlightDisplay = () => {
  ...
  const handleSearchModeChange = (mode) => {
    ...
    if (mode === 'agency') {
     ...
     // ============================================
// FILTRAGE DES VOLS ALLER PAR AGENCE/COMPAGNIE
// ============================================
useEffect(() => {
  if (!agencyId) {
    setFilteredVols([]);
  ...
  // Filtrer les vols par agence
  let agencyVols = allVols.filter(vol => vol.agencyId === agencyId);
  
  // Appliquer le filtre compagnie si sélectionné
  if (selectedCompany && filterByCompany) {
    agencyVols = agencyVols.filter(vol => vol.companyName === selectedCompany.name);
  }

  // Séparer les vols de l'agence sélectionnée des autres
  const otherAgencyVols = allVols.filter(vol => vol.agencyId !== agencyId);
...
}, [agencyId, showAllAgencyFlights, allVols, selectedCompany, filterByCompany]);
...
useEffect(() => {
  ...
  // Filtrer les vols retour par agence
  let agencyVols = allVols.filter(vol => vol.agencyId === agencyId);
...
}, [agencyId, showAllReturnAgencyFlights, allVols, selectedCompany, filterByCompany, tripType]);
  // ============================================
    ...
    const fetchVols = async () => {
      ...
      try {
        ...
        if (searchMode === 'company' && selectedCompany) {
          volsToShow = transformedVols.filter(v => v.companyName === selectedCompany.name);
        }
  ...
  const handleCompanySelection = (company) => {
    ...
    const filteredVols = allVols.filter(v => v.companyName === company.name);
  ...
  // ============================================
  // GESTION DES SÉLECTIONS (un seul clic)
  // ============================================
  ...
  // ============================================
  // RENDU PRINCIPAL DE ManualFlightSelection
  // ============================================
  return (
...
<div>
  <label className="block text-sm font-medium text-gray-700 mb-2 flex items-center gap-2">
    ...
    {agencyId && (
      <span className="ml-2 text-xs bg-blue-100 text-blue-800 px-2 py-1 rounded-full">
        {showAllAgencyFlights ? 'Autres agences' : 'Agence sélectionnée'}
      </span>
    )}
  </label>
  ...
  <div className="relative">
    ...
    <input
      ...
      onFocus={() => {
        if (!selectionLocked && vols.length > 0) {
          const isSelectedFlightText =
            selectedFlight &&
        ...
        } else if (vols.length > 0 && !selectedFlight) {
          setShowVolSuggestions(true);
        }
      }}
      onBlur={() => {
    ...
    {/* ✅ LISTE COLLEE A L INPUT */}
    {!selectionLocked && showVolSuggestions && vols.length > 0 && (
      <div className="absolute top-full left-0 w-full bg-white border border-gray-300 rounded-lg shadow-lg mt-1 max-h-64 overflow-hidden z-50">
        ...
        <div className="flex justify-between items-center p-3 bg-gray-50 border-b">
          ...
          <button
            type="button"
            onClick={() => setShowVolSuggestions(false)}
  ...
  {/* message agence sans vol */}
  {agencyId && agencyVolCount === 0 && !showAllAgencyFlights && (
    <div className="mt-3 p-3 bg-yellow-50 border border-yellow-200 rounded-md">
      ...
      >
        ...
        Voir les vols des autres agences
          ...
          {/* Vol retour - toujours affiché si aller-retour */}
{tripType === 'round-trip' && (
  <div className="mt-4">
    <label className="block text-sm font-medium text-gray-700 mb-2 flex items-center gap-2">
      <FontAwesomeIcon icon={faPlaneArrival} className="text-purple-500 text-lg" />
      Sélectionner un vol retour

      {agencyId && (
        <span className="ml-2 text-xs bg-purple-100 text-purple-800 px-2 py-1 rounded-full">
          {showAllReturnAgencyFlights ? 'Autres agences' : 'Agence sélectionnée'}
        </span>
      )}
    </label>
    ...
    <div className="relative">
      ...
      <input
        ...
        onFocus={() => {
          if (!selectionLocked && returnVols.length > 0) {
            const isSelectedReturnFlightText =
              selectedReturnFlight &&
          ...
          } else if (returnVols.length > 0 && !selectedReturnFlight) {
            setShowReturnVolSuggestions(true);
          }
        }}
        onBlur={() => {
      ...
      {!selectionLocked && showReturnVolSuggestions && returnVols.length > 0 && (
        <div className="absolute top-full left-0 w-full bg-white border border-gray-300 rounded-lg shadow-lg mt-1 max-h-64 overflow-hidden z-50">
          ...
          <div className="flex justify-between items-center p-3 bg-gray-50 border-b">
            <span className="text-xs text-gray-600">
              {returnVols.length} vol(s) disponible(s)
            </span>

            <button
              type="button"
              onClick={() => setShowReturnVolSuggestions(false)}
              className="text-gray-400 hover:text-gray-600"
            >
              <FontAwesomeIcon icon={faTimes} />
            </button>
    ...
    {/* message agence sans vol retour */}
    {agencyId && agencyReturnVolCount === 0 && !showAllReturnAgencyFlights && (
      <div className="mt-3 p-3 bg-yellow-50 border border-yellow-200 rounded-md">
        <p className="text-sm text-yellow-700 mb-2">
    ...
    {/* affichage du vol retour sélectionné */}
    {selectedReturnFlight && (
      <div className="mt-3 p-3 bg-purple-50 rounded-md border border-purple-200">
        <div className="flex items-center gap-2">
          <FontAwesomeIcon icon={faPlaneArrival} className="text-purple-600 text-sm flex-shrink-0" />
          <div className="flex-1 min-w-0">
            <p className="text-sm font-semibold text-gray-800 truncate">
              Vol retour sélectionné: <span className="text-purple-700">{selectedReturnFlight.flightName}</span>
            </p>
            <p className="text-xs text-gray-600 truncate">
              {selectedReturnFlight.companyName} • {selectedReturnFlight.origin} → {selectedReturnFlight.destination}
            </p>
            <p className="text-xs text-gray-500 mt-1">
              <FontAwesomeIcon icon={faCalendarAlt} className="mr-1" />
              Départ: {formatDate(selectedReturnFlight.departureTime)}
            </p>
          </div>
        </div>
      </div>
...
const CreateReservationCampaign = () => {
  ...
  const handleManualFlightSelect = (flight) => {
    ...
    if (flight?.departureTime) {
      const formattedDate = new Date(flight.departureTime).toISOString().split('T')[0];
      setManualStartAt(formattedDate);
                ...
                {!campaignHasFlight && addManualFlight && manuallySelectedFlight && (
                  <div className="p-3 bg-green-50 rounded-lg">
                    ...
                    {manualStartAt && (
                      <p className="text-xs text-gray-500 mt-1">
                        <FontAwesomeIcon icon={faCalendarAlt} className="mr-1" />
                        Départ: {new Date(manualStartAt).toLocaleDateString('fr-FR')}
                      </p>
                    )}
                  </div>
                )}
