...
const ProfessionalReservationForm = () => {
  ...
  const [classes, setClasses] = useState([]); // Classes globales
  ...
  const [selectedClass, setSelectedClass] = useState(null); // Classe globale sélectionnée
  ...
  const loadInitialData = async () => {
    ...
    try {
      await Promise.all([
        ...
        loadFlightAgencies(),
        loadClassAgencies(),
        loadClasses(), // Charge les classes globales
        loadPricingRules(),
        loadProfile()
      ]);
    } catch (err) {
  ...
  // ✅ Chargement des classes globales
  const loadClasses = async () => {
    try {
      const response = await classeService.getClasses({ status: 'active' });
      const data = response?.data || response || [];
      setClasses(Array.isArray(data) ? data : []);
      console.log('🎓 Classes globales chargées:', data);
    } catch (error) {
      console.error('❌ Erreur classes globales:', error);
      setClasses([]);
    }
  };
  ...
  // ✅ Classes globales disponibles (sans filtrage par vol)
  const availableGlobalClasses = useMemo(() => {
  ...
  // ✅ Sélection de la classe globale
  const handleClassSelect = (classItem) => {
    console.log('🎫 Classe globale sélectionnée:', classItem);
  ...
  // ============================================================================
  useEffect(() => {
    if (!selectedOutboundFlight || !selectedClass) {
      setTotalPrice(0);
      return;
    }
    ...
    const rulesMap = {};
    pricingRules
      .filter(rule => rule.classId === selectedClass.id)
      .forEach(rule => {
    ...
    passengers.forEach(passenger => {
      ...
      // Ajouter le prix du retour si aller-retour (même classe)
      if (formData.tripType === 'round-trip' && selectedReturnFlight) {
        passengerPrice += basePrice; // Même prix pour le retour
      }
  ...
  }, [selectedOutboundFlight, selectedReturnFlight, selectedClass, passengers, formData.tripType, pricingRules]);
              ...
              {!selectedOutboundFlight ? (
                ...
                <>
                  ...
                  <Dropdown isOpen={activeDropdown === 'class'} onClose={() => setActiveDropdown(null)}>
                    {availableGlobalClasses.length > 0 ? (
                      availableGlobalClasses.map(classItem => (
                        <DropdownItem key={classItem.id} onClick={() => handleClassSelect(classItem)}>
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                <span className="font-medium text-gray-800">{classItem.name}</span>
                            ...
                            {classItem.price > 0 && (
                              <span className="font-semibold text-green-600">
                                {parseFloat(classItem.price).toLocaleString()} FCFA
                              </span>
                            )}
                          </div>
                        </DropdownItem>
                ...
                <div className="flex justify-between items-center">
                  <div>
                    ...
                    {selectedClass.description && (
                      <p className="text-xs text-gray-600 mt-1">{selectedClass.description}</p>
                    )}
                  </div>
                  {selectedClass.price > 0 && (
                    <p className="font-bold text-green-600">
                      {parseFloat(selectedClass.price).toLocaleString()} FCFA
                    </p>
                  )}
                </div>
              </div>
