...
export const ManualFlightSelection = ({
  ...
  // Fonction pour obtenir le nom de la compagnie
  ...
  // Fonction pour obtenir l'ID de la compagnie
  ...
  // Fonction pour obtenir le nom de la compagnie par ID
  ...
  // Fonction pour filtrer les vols par compagnie
  const filterVolsByCompany = (volsList, companyId) => {
    if (!companyId) return volsList;
    return volsList.filter(vol => {
      // Vérifier toutes les sources possibles de companyId
      return vol.companyId === companyId || 
             vol.flight?.companyId === companyId ||
             vol.flight?.company?.id === companyId ||
             vol.flight?.companyVol?.id === companyId;
    });
  };

  // ============================================
  // RECHERCHE DE VOLS ALLER - VERSION CORRIGÉE
  // ============================================
  useEffect(() => {
    const fetchVols = async () => {
      ...
      try {
        ...
        console.log('📊 Données extraites brutes:', volsData);
        ...
        // Transformation des données avec extraction correcte des IDs
        const transformedVols = volsData.map(vol => {
          // Extraire les informations de vol
          const flightData = vol.flight || {};
          
          // Extraire les informations de compagnie (plusieurs structures possibles)
          const companyData = flightData.company || flightData.companyVol || {};
          const companyId = companyData.id || flightData.companyId || null;
          const companyName = companyData.name || 'Compagnie inconnue';
          
          // Extraire les informations d'origine
          const originData = flightData.origin || {};
          const originId = originData.id || flightData.originId || null;
          const originName = originData.name || originData.city || 'Inconnue';
          
          // Extraire les informations de destination
          const destinationData = flightData.destination || {};
          const destinationId = destinationData.id || flightData.destinationId || null;
          const destinationName = destinationData.name || destinationData.city || 'Inconnue';
          ...
          return {
            ...
            agencyId: vol.agency?.id || vol.agencyId || null,
            flight: flightData,
            agency: vol.agency
          };
        });
        
        console.log('✅ Vols transformés avec IDs:', transformedVols.map(v => ({
          id: v.id,
          name: v.flightName,
        ...
        if (selectedCompany) {
          volsToShow = transformedVols.filter(v => 
            v.companyId === selectedCompany.id ||
            v.flight?.companyId === selectedCompany.id ||
            v.flight?.company?.id === selectedCompany.id ||
            v.flight?.companyVol?.id === selectedCompany.id
          );
          console.log(`🔍 Vols filtrés pour compagnie ${selectedCompany.name}:`, volsToShow);
        }
  ...
  }, [volSearch, agencyId, selectedCompany]);
  ...
  useEffect(() => {
    const fetchReturnVols = async () => {
      ...
      try {
        ...
        // Transformation avec la même logique que pour les vols aller
        const transformedVols = volsData.map(vol => {
          const flightData = vol.flight || {};
          const companyData = flightData.company || flightData.companyVol || {};
          const originData = flightData.origin || {};
...
const handleCompanySelection = (company) => {
  ...
  const filteredVols = allVols.filter(v => {
    const match = v.companyId === company.id ||
                  ...
                  v.flight?.companyVol?.id === company.id;
    
    if (match) console.log(`✅ Vol correspondant trouvé:`, v);
    return match;
  });
  ...
  // IMPORTANT: Forcer l'affichage des suggestions
...
const CreateReservationCampaign = () => {
    ...
    // ✅ Nouvel état pour contrôler si l'utilisateur souhaite ajouter un vol manuellement
    const [addManualFlight, setAddManualFlight] = useState(false);
    
    // États pour la sélection manuelle de vol (quand campagne sans vol et addManualFlight=true)
    ...
    const handleCampaignChange = async (campaignId) => {
        if (!campaignId) {
            ...
            setAddManualFlight(false); // Reset de l'option manuelle
            setFormData({
                ...formData,
                campaignId: '',
                agencyId: ''
        ...
        try {
            ...
            if (hasFlight && campaign.volDetails) {
                ...
                // Si la campagne n'a pas de vol, réinitialiser l'option manuelle
    ...
    // Fonction pour réinitialiser les sélections manuelles
    ...
    // Gestionnaires pour la sélection manuelle (quand campagne sans vol et addManualFlight=true)
    ...
    // ✅ Validation avant soumission - avec option manuelle
    const validateForm = () => {
        ...
        // Si la campagne a un vol, pas besoin de validation manuelle
        if (campaignHasFlight) {
            // Tout est bon
        } 
        // Si la campagne n'a pas de vol et que l'utilisateur a choisi d'ajouter un vol
        ...
        // Si la campagne n'a pas de vol et que l'utilisateur n'a pas choisi d'ajouter un vol
        // → Pas de validation, on utilise uniquement les informations de la campagne
        
        // Validation des passagers (toujours requise)
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // Construire le payload final selon le type de campagne et le choix de l'utilisateur
            let payload = {
                ...formData,
            ...
            // Cas 1: Campagne avec vol inclus
            if (campaignHasFlight) {
                payload = {
                    ...payload,
            ...
            // Cas 2: Campagne sans vol mais l'utilisateur a choisi d'ajouter un vol
            else if (addManualFlight) {
                payload = {
                    ...payload,
                    ...
                    startDestinationId: manuallySelectedFlight?.originId || '',
                    endDestinationId: manuallySelectedFlight?.destinationId || '',
                    flightSource: 'manual'
                };
            }
            // Cas 3: Campagne sans vol et l'utilisateur n'a pas ajouté de vol
            else {
                payload = {
                    ...payload,
                    agencyVolId: '',
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-2">
                        <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-lg overflow-hidden">
                            ...
                            {/* Section pour les campagnes sans vol - Choix d'ajouter un vol ou non */}
                                    ...
                                    <div className="space-y-4">
                                        <div className="flex items-start gap-4 p-4 bg-white rounded-lg border border-gray-200">
                                            <input
                                                type="checkbox"
                                                id="addManualFlight"
                                                checked={addManualFlight}
                                                onChange={(e) => {
                                                    setAddManualFlight(e.target.checked);
                                                    if (!e.target.checked) {
                                                        resetManualSelections();
                                                    }
                                                }}
                                            ...
                                            />
                                            <div>
                                                <label htmlFor="addManualFlight" className="font-medium text-gray-800 cursor-pointer">
                                                    Ajouter un vol à cette campagne
                                                </label>
                                                <p className="text-sm text-gray-600 mt-1">
                                                    Cochez cette case si vous souhaitez ajouter un vol spécifique à cette réservation.
                                                    Si vous ne cochez pas, seule la campagne sera réservée.
                                                </p>
                                            </div>
                                        </div>

                                        {/* Affichage du formulaire de sélection de vol uniquement si coché */}
                                        {addManualFlight && (
                                            <div className="mt-4">
                            ...
                            <div className="p-6 border-b border-gray-200">
                                <div className="flex justify-between items-center mb-4">
                                    <h2 className="text-lg font-semibold text-gray-900 flex items-center gap-2">
                                        ...
                                        {selectedCampaign && !campaignHasFlight && !addManualFlight 
                    ...
                    <div className="lg:col-span-1">
                        <div className="bg-white rounded-xl shadow-lg p-6 sticky top-6">
                            ...
                            <div className="space-y-4">
                                ...
                                {/* Résumé vol si campagne sans vol et ajout manuel */}
                                ...
                                {/* Résumé classe si campagne sans vol et ajout manuel */}
