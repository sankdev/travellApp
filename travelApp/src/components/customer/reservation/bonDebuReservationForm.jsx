...
const ReservationForm = () => {
  ...
  // États pour la compagnie (optionnel)
  ...
  // Filtrage des vols aller en fonction de l'agence sélectionnée et optionnellement de la compagnie
  useEffect(() => {
    if (!selectedAgency) {
      setFilteredVols([]);
      setVols([]);
    ...
    // Commencer avec tous les vols de l'agence sélectionnée
    let agencyFlights = allVols.filter(vol => vol.agencyId === selectedAgency.id);
    
    // Filtrer par compagnie si sélectionnée et si le filtre est activé
    ...
    // Réinitialiser le message de recherche
    setSearchMessage('');
  }, [selectedAgency, selectedCompany, filterByCompany, allVols]);

  // Filtrage des vols retour en fonction de l'agence sélectionnée
  useEffect(() => {
    if (!selectedAgency) {
  ...
  // Gestion de la compagnie optionnelle
  ...
  const handleCompanySelection = (company) => {
    ...
    setShowCompanySuggestions(false);
    setFilterByCompany(true); // Activer le filtre par compagnie
  ...
  const clearCompanySelection = () => {
    ...
    setCompanyVols([]);
    setFilterByCompany(false); // Désactiver le filtre par compagnie
  };
  ...
  // ============================================
  // CALCUL DU PRIX TOTAL - LOGIQUE UNIFIÉE
  // ============================================
  ...
  const handleFlightSelection = (vol) => {
    ...
    setFormData(prev => ({
      ...
      // ✅ NE PAS mettre à jour startAt automatiquement
    }));
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-medium">
                      Filtre actif: <span className="text-indigo-700">{selectedCompany.name}</span>
                    </span>
                  </div>
                  <button
                    type="button"
                  ...
                  >
                    <FontAwesomeIcon icon={faTimes} /> Retirer le filtre
                  </button>
                </div>
                <p className="text-xs text-gray-600 mt-1">
                  {agencyVolCount} vol(s) trouvé(s) pour cette compagnie
                </p>
              </div>
            )}
          </div>
