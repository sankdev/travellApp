...
const ManualFlightSelection = ({
  ...
  const formatFlightDisplay = (vol) => {
    return `${vol.flightName} - ${vol.companyName} - ${vol.origin || '?'} → ${vol.destination || '?'} - ${vol.price} FCFA`;
  };
    ...
    <div className="space-y-6 p-4 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl border-2 border-blue-200">
      ...
      {(selectedFlight || selectedReturnFlight) && (
        <div className="mt-4 p-3 bg-blue-100 rounded-lg">
          <p className="text-sm font-medium text-blue-800 mb-2">Résumé de votre sélection :</p>
          {selectedFlight && (
            <p className="text-xs text-blue-700">
              ✈️ Aller : {selectedFlight.flightName} - {selectedFlight.price} FCFA
            </p>
          )}
          {selectedReturnFlight && (
            <p className="text-xs text-blue-700 mt-1">
              🔙 Retour : {selectedReturnFlight.flightName} - {selectedReturnFlight.price} FCFA
            </p>
          )}
          {selectedClass && (
            <p className="text-xs text-blue-700 mt-1">
              🎫 Classe : {selectedClass.class?.name} (x{selectedClass.priceMultiplier})
            </p>
          )}
        </div>
      )}
...
const CreateReservationCampaign = () => {
    ...
    }, [selectedCampaign, manuallySelectedFlight, manuallySelectedReturnFlight, 
        manuallySelectedClass, manualTripType, passengers.length]);
