...
const CreateReservation = () => {
    ...
    useEffect(() => {
        if (selectedFlightAgency) {
            ...
            // S'assurer que classAgencies est un tableau
            ...
            // Si des classes sont trouvées et qu'aucune n'est sélectionnée, sélectionner la première
            ...
            // Afficher un message si le vol n'a pas de classes
    ...
    useEffect(() => {
        if (selectedClassAgency) {
            ...
            // S'assurer que pricingRules est un tableau
            const rulesList = Array.isArray(pricingRules) ? pricingRules : [];
            
            // Filtrer les règles pour cette classe
...
useEffect(() => {
    const calculateTotalPrice = () => {
        ...
        // Prix de base de la classe (prix du vol)
        const classPrice = parseFloat(selectedClassAgency.price) || 0;
        console.log('💰 Prix du vol (classe):', classPrice);

        // Créer un map des règles pour un accès rapide
        const rulesMap = {};
        ...
        // Calcul du prix aller pour TOUS les passagers
        let totalPrice = 0;
        ...
        passengers.forEach((passenger, index) => {
            ...
            // ✅ TOUJOURS ajouter le prix du vol pour chaque passager
            totalPrice += classPrice;
            console.log(`  ➕ Prix du vol: +${classPrice} FCFA`);
            
            // ✅ AJOUTER la règle UNIQUEMENT si le passager n'est pas adulte
            if (passengerType !== 'ADLT') {
                const rulePrice = rulesMap[passengerType];
                if (rulePrice) {
                    totalPrice += rulePrice;
                    console.log(`  ➕ Règle ${passengerType === 'CHD' ? 'enfant' : 'bébé'}: +${rulePrice} FCFA`);
                } else {
        ...
        if (formData.tripType === 'round-trip' && formData.returnVolId) {
            ...
            if (returnClassAgency) {
                ...
                console.log('💰 Prix retour trouvé:', returnClassPrice);
                ...
                passengers.forEach((passenger, index) => {
                    ...
                    // ✅ TOUJOURS ajouter le prix du vol retour
                    totalPrice += returnClassPrice;
                ...
                totalPrice += classPrice * passengers.length;
            }
        }
    ...
    const handleFlightSelection = (vol) => {
        ...
        // Vérifier si ce vol a des classes disponibles
