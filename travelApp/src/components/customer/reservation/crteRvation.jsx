...
const CreateReservation = () => {
    ...
    // ===========================================
      // ============================================
// CALCUL DU PRIX TOTAL - CORRIGÉ POUR ALLER-RETOUR
// ============================================
useEffect(() => {
    const calculateTotalPrice = () => {
        ...
        console.log('Règles filtrées:', filteredRules);

        const classPrice = parseFloat(selectedClassAgency.price) || 0;
        console.log('💰 Prix du vol (classe) aller:', classPrice);

        const rulesMap = {};
        filteredRules.forEach(rule => {
            rulesMap[rule.typePassenger] = parseFloat(rule.price) || 0;
        });
        console.log('🗺️ Map des règles aller:', rulesMap);
        ...
        passengers.forEach((passenger, index) => {
            ...
            console.log(`👤 Passager ${index + 1} (aller) - Type: ${passengerType}`);
            
            // Prix du vol aller
            let passengerPrice = classPrice;
            console.log(`  ➕ Prix aller: +${classPrice} FCFA`);
            
            // Ajouter la règle si nécessaire
            if (passengerType !== 'ADLT') {
                const rulePrice = rulesMap[passengerType];
                if (rulePrice) {
                    passengerPrice += rulePrice;
                    console.log(`  ➕ Règle aller ${passengerType === 'CHD' ? 'enfant' : 'bébé'}: +${rulePrice} FCFA`);
                }
            }
            
            totalPrice += passengerPrice;
            console.log(`  = Total aller pour ce passager: ${passengerPrice} FCFA`);
        });

        console.log('💰 Prix total aller:', totalPrice);

        // ✅ AJOUTER le prix RETOUR si aller-retour
        if (formData.tripType === 'round-trip') {
            console.log('🔄 Traitement aller-retour - AJOUT DU PRIX RETOUR');
            ...
            if (returnClassAgency) {
                ...
                console.log('💰 Prix du vol retour:', returnClassPrice);
                ...
                console.log('🗺️ Map des règles retour:', returnRulesMap);
                
                // Ajouter le prix retour pour chaque passager
                passengers.forEach((passenger, index) => {
                    const passengerType = passenger.typePassenger;
                    console.log(`🔄 Retour passager ${index + 1} - Type: ${passengerType}`);
                    
                    // Prix du vol retour
                    let returnPrice = returnClassPrice;
                    console.log(`  ➕ Prix retour: +${returnClassPrice} FCFA`);
                    
                    // Ajouter la règle retour si nécessaire
                    if (passengerType !== 'ADLT') {
                        const rulePrice = returnRulesMap[passengerType];
                        if (rulePrice) {
                            returnPrice += rulePrice;
                            console.log(`  ➕ Règle retour ${passengerType === 'CHD' ? 'enfant' : 'bébé'}: +${rulePrice} FCFA`);
                        }
                    }
                    
                    totalPrice += returnPrice;
                    console.log(`  = Total retour pour ce passager: ${returnPrice} FCFA`);
                });
            } else {
                console.log('⚠️ Aucune classe trouvée pour le vol retour');
                // Fallback: utiliser le même prix que l'aller pour le retour
                const returnPrice = classPrice * passengers.length;
                totalPrice += returnPrice;
                console.log(`  ➕ Prix retour (fallback): +${returnPrice} FCFA`);
            }
        }

        console.log('💰 PRIX TOTAL FINAL (aller + retour):', totalPrice);
        setTotalPrice(totalPrice);
        setFormData(prev => ({ ...prev, totalPrice }));
    };

    calculateTotalPrice();
}, [selectedFlightAgency, selectedClassAgency, passengers, formData.tripType, formData.returnVolId, classAgencies, pricingRules, filteredRules]);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const cleanedFormData = {
                ...
                returnVolId: formData.tripType === 'one-way' ? null : (formData.returnVolId || null),
                                                    ...
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <div>
                                                            ...
                                                            <p className="text-sm text-gray-600">
                                                                Prix: <span className="font-bold text-green-600">{price} FCFA</span>
                                                            </p>
                                                            {filteredRules.length > 0 && (
                                                                <div className="mt-1 text-xs text-gray-500">
                                                                    {filteredRules.map((rule, idx) => (
                                                                        <span key={idx} className="mr-2">
                                                                            {rule.typePassenger === 'CHD' ? '👶 Enfant' : '🍼 Bébé'}: {parseFloat(rule.price).toLocaleString()} FCFA
                                                                        </span>
                                                                    ))}
                                                                </div>
                                                            )}
                            ...
                            {formData.tripType === 'round-trip' && (
                                <div className="md:col-span-2">
                                    ...
                                    <input
                                        ...
                                        className={inputClassName}
                                    />
                                    {!formData.endAt && formData.tripType === 'round-trip' && (
