...
// Composant pour la sélection manuelle de vol (aller ET retour)
const ManualFlightSelection = ({
  ...
  const calculateTotalPrice = () => {
    ...
    if (selectedFlight) {
      pricePerPerson += selectedFlight.price || 0;
    }
    
    if (tripType === 'round-trip' && selectedReturnFlight) {
      pricePerPerson += selectedReturnFlight.price || 0;
    }
    
    if (selectedClass && selectedFlight) {
      pricePerPerson = pricePerPerson * (selectedClass.priceMultiplier || 1);
    }
  ...
  useEffect(() => {
    const fetchVols = async () => {
      ...
      try {
        ...
        console.error('❌ Erreur chargement vols aller:', error);
        setVols([]);
      } finally {
        setLoading(false);
      }
  ...
  // Recherche de vols retour (indépendante)
  useEffect(() => {
    const fetchReturnVols = async () => {
      ...
      try {
        ...
        // Enrichir les données des vols retour
        volsData = volsData.map(vol => ({
          ...vol,
          flightName: vol.flight?.name || 'Vol sans nom',
          companyName: getCompanyName(vol.flight?.companyId),
      ...
      try {
        ...
        if (response && Array.isArray(response)) {
          const classesForFlight = response.filter(cls => 
            cls.agencyVolId === parseInt(selectedFlight.id) || 
            cls.agencyVolId === selectedFlight.id
          );
  ...
  const formatFlightDisplay = (vol) => {
    if (!vol) return '';
    return `${vol.flightName || 'Vol'} - ${vol.companyName || 'Compagnie'} - ${vol.origin || '?'} → ${vol.destination || '?'} - ${vol.price || 0} FCFA`;
  };
  ...
  return (
    <div className="space-y-6 p-4 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl border-2 border-blue-200">
      ...
      <div>
        ...
        <div className="relative">
          <input
            ...
            onFocus={() => vols.length > 0 && setShowVolSuggestions(true)}
        ...
        <div>
          ...
          <div className="relative">
            <input
              ...
              onFocus={() => returnVols.length > 0 && setShowReturnVolSuggestions(true)}
          ...
          {showReturnVolSuggestions && returnVols.length > 0 && (
            <ul className="absolute z-50 mt-1 w-full bg-white border border-gray-300 rounded-lg shadow-lg max-h-60 overflow-y-auto">
              {returnVols
                .filter(v => v.id !== selectedFlight?.id) // Éviter le même vol que l'aller
                .map(vol => (
                  <li
                    key={vol.id}
      ...
      {/* Classes disponibles - UNIQUEMENT si un vol aller est sélectionné */}
      {selectedFlight && classes.length > 0 && (
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">
            Classe *
          </label>
          <div className="relative">
            <input
              ...
              onFocus={() => filteredClasses.length > 0 && setShowClassSuggestions(true)}
                ...
                >
                  <div className="flex justify-between items-center">
                    <div>
                      ...
                      <p className="text-xs text-gray-500 mt-1">{cls.class?.description}</p>
                    </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="relative">
          ...
          <input
            ...
            className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
            placeholder="Départ"
            readOnly={!!selectedFlight} // Read-only si un vol est sélectionné
          />
          {showStartDestinationSuggestions && startDestinations.length > 0 && !selectedFlight && (
        ...
        <div className="relative">
          ...
          <input
            ...
            readOnly={!!selectedFlight} // Read-only si un vol est sélectionné
          />
          {showEndDestinationSuggestions && endDestinations.length > 0 && !selectedFlight && (
            <ul className="absolute z-10 w-full mt-1 bg-white border border-gray-300 rounded-lg shadow-lg max-h-60 overflow-y-auto">
      ...
      {(selectedFlight || selectedReturnFlight) && (
        <div className="mt-4 p-4 bg-gradient-to-r from-green-50 to-emerald-50 rounded-lg border border-green-200">
          <p className="text-sm font-medium text-gray-700 mb-2">Prix estimé :</p>
          <div className="space-y-2">
            {selectedFlight && (
              <p className="text-sm text-gray-600 flex justify-between">
                <span>Vol aller (par personne) :</span>
                <span className="font-medium">{selectedFlight.price} FCFA</span>
              </p>
            )}
            {tripType === 'round-trip' && selectedReturnFlight && (
              <p className="text-sm text-gray-600 flex justify-between">
                <span>Vol retour (par personne) :</span>
                <span className="font-medium">{selectedReturnFlight.price} FCFA</span>
              </p>
            )}
            {selectedClass && (
              <p className="text-sm text-gray-600 flex justify-between">
                <span>Multiplicateur de classe :</span>
                <span className="font-medium">x{selectedClass.priceMultiplier}</span>
              </p>
            )}
            <div className="pt-2 border-t border-green-200">
              <p className="text-base font-bold text-green-600 flex justify-between">
                <span>Total pour {passengersCount} passager(s) :</span>
...
const CreateReservationCampaign = () => {
    ...
    const calculateTotalPrice = () => {
        ...
        // Prix de la campagne (par personne)
        if (selectedCampaign) {
            pricePerPerson += selectedCampaign.price || 0;
        }
        
        // Prix des vols sélectionnés manuellement (par personne)
        if (manuallySelectedFlight) {
            pricePerPerson += manuallySelectedFlight.price || 0;
        }
        
        if (manualTripType === 'round-trip' && manuallySelectedReturnFlight) {
            pricePerPerson += manuallySelectedReturnFlight.price || 0;
        }
        ...
        // Prix total = prix par personne × nombre de passagers
    ...
    useEffect(() => {
        ...
        selectedCampaign, 
        manuallySelectedFlight, 
        manuallySelectedReturnFlight, 
    ...
    const handleManualReturnFlightSelect = (flight) => {
        ...
        setFormData(prev => ({ 
            ...prev, 
            returnVolId: flight ? flight.id : '' 
        }));
    };

    const handleManualClassSelect = (cls) => {
        setManuallySelectedClass(cls);
        setFormData(prev => ({ ...prev, agencyClassId: cls ? cls.id : '' }));
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const payload = {
                ...
                startDestinationId: campaignHasFlight ? formData.startDestinationId : (manuallySelectedFlight?.originId || ''),
                endDestinationId: campaignHasFlight ? formData.endDestinationId : (manuallySelectedFlight?.destinationId || ''),
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="p-4 bg-gradient-to-br from-green-50 to-emerald-50 rounded-lg border border-green-200">
                                    ...
                                    <div className="mt-2 text-xs text-gray-500">
                                        ...
                                        {selectedCampaign && (
                                            <p>• Campagne: {selectedCampaign.price.toLocaleString()} FCFA/pers</p>
                                        )}
                                        {manuallySelectedFlight && (
                                            <p>• Vol aller: {manuallySelectedFlight.price.toLocaleString()} FCFA/pers</p>
                                        )}
                                        {manualTripType === 'round-trip' && manuallySelectedReturnFlight && (
                                            <p>• Vol retour: {manuallySelectedReturnFlight.price.toLocaleString()} FCFA/pers</p>
                                        )}
                                        {manuallySelectedClass && (
                                            <p>• Multiplicateur classe: x{manuallySelectedClass.priceMultiplier}</p>
                                        )}
                                    </div>
                                </div>
                                    ...
                                    <div className="p-3 bg-purple-50 rounded-lg">
                                        ...
                                        {manualTripType === 'round-trip' && manuallySelectedReturnFlight && (
                                            <div className="mt-2 pt-2 border-t border-purple-200">
                                                <p className="text-xs font-medium text-purple-800 mb-1">Vol retour:</p>
                                                <p className="text-sm">{manuallySelectedReturnFlight.flightName}</p>
                                                <p className="text-xs text-gray-600">
                                                    {manuallySelectedReturnFlight.origin} → {manuallySelectedReturnFlight.destination}
                                                </p>
                                            </div>
                                        )}
                                    </div>
                                ...
                                <div className="mt-4 text-xs text-gray-500 space-y-1">
                                    ...
                                    {showManualFlightSelection && (
                                        <p className="flex items-center gap-1">
                                            <FontAwesomeIcon icon={faInfoCircle} className="text-blue-400" />
                                            {manualTripType === 'round-trip' 
                                                ? 'Vol aller ET vol retour requis' 
                                                : 'Vol aller requis'}
                                        </p>
                                    )}
                                </div>
