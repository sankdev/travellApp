...
const ReservationForm = () => {
  ...
  const [classAgencies, setClassAgencies] = useState([]); // Renommé pour clarté
  ...
  // Filtrage des classes disponibles pour le vol sélectionné
  useEffect(() => {
    if (selectedVol) {
      const availableClasses = classAgencies.filter(ca => 
        ca.agencyVolId === selectedVol.id && ca.status === 'active'
      );
  ...
  // Filtrage des classes pour le vol retour
  useEffect(() => {
    if (classSearch.length > 0) {
      const filtered = classAgencies.filter((cls) =>
        cls.class?.name?.toLowerCase().includes(classSearch.toLowerCase())
      );
      setFilteredClasses(filtered);
    }
  }, [classSearch, classAgencies]);
  ...
  // Calcul du prix total avec gestion des règles par type de passager
  const handlePriceCalculation = useCallback(async () => {
    try {
      ...
      if (!selectedVol || !selectedClassAgency) {
        console.log('❌ Vol ou classe non sélectionné');
        setTotalPrice(0);
        return;
      }

      // Prix de base pour un adulte (depuis ClassAgency.price)
      ...
      // Traiter chaque passager pour le vol aller
      for (const passenger of passengers) {
        ...
        if (passengerType === 'ADLT') {
          ...
          // Chercher une règle spécifique pour enfant/bébé
          ...
          for (const passenger of passengers) {
            ...
            if (passengerType === 'ADLT') {
              ...
              const returnRule = pricingRules?.find(r => 
                r.agencyVolId === selectedReturnVol.id &&
  ...
  }, [selectedVol, selectedReturnVol, selectedClassAgency, passengers, formData.tripType, handlePriceCalculation]);
  ...
  const handleFlightSelection = (vol) => {
    ...
    setFormData(prev => ({
      ...
      startAt: vol.departureTime ? new Date(vol.departureTime).toISOString().split('T')[0] : ''
    }));
              ...
              <div className="grid grid-cols-1 gap-3">
                {filteredClasses.map((classAgency) => {
                  ...
                  const rules = pricingRules.filter(r => 
                    r.agencyVolId === selectedVol.id && 
                    r.agencyClassId === classAgency.id
                  );
                            ...
                            <div>
                              ...
                              <div className="flex items-center gap-4 mt-2">
                                <span className="text-lg font-bold text-green-600">
                                  {parseFloat(classAgency.price).toLocaleString()} FCFA
                                </span>
                                <span className="text-xs text-gray-500">
                                  par adulte
                                </span>
                    ...
                    <p className="text-sm">
                      ...
                      {passenger.typePassenger === 'ADLT' ? (
                        ...
                        (() => {
                          ...
                          const price = rule ? parseFloat(rule.price) : parseFloat(selectedClassAgency.price);
                          return (
                            <span className="text-green-600 font-bold">
