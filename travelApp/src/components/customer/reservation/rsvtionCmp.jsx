...
const ManualFlightSelection = ({
    ...
    const fetchAllClasses = async () => {
      ...
      try {
        ...
        if (response && Array.isArray(response)) {
          const enrichedClasses = response.map(cls => ({
            ...cls,
            className: cls.class?.name || 'Classe sans nom',
  ...
  useEffect(() => {
    const fetchVols = async () => {
      ...
      try {
        ...
        // ✅ NE PAS réinitialiser vols si on est en mode filtre compagnie
        if (filterByCompany && selectedCompany) {
      const filtered = volsData.filter(vol => vol.companyId === selectedCompany.id);
      setVols(filtered);
    } else {
      setVols(volsData);
    }
  ...
  }, [volSearch, agencyId, filterByCompany, selectedCompany]);
  ...
  // ✅ Fonction de recherche de compagnie CORRIGÉE
  ...
  // ✅ Sélection d'une compagnie CORRIGÉE
  const handleCompanySelection = (company) => {
    ...
    setVols(companyVolsFiltered);
  };

  // ✅ Effacer la sélection de compagnie CORRIGÉ
  ...
  // ✅ GESTIONNAIRE DE SÉLECTION DE VOL CORRIGÉ
   // Ajoutez cet état dans le composant

// Modifiez handleFlightSelect
const handleFlightSelect = (vol) => {
  if (!vol || selectionLocked) return; // Empêcher les doubles clics
...
useEffect(() => {
  // NE PAS RÉOUVRIR PENDANT LE VERROUILLAGE
  if (selectionLocked) {
    return;
  }
  
  // Gérer l'ouverture des suggestions pour le vol retour
  if (returnVolSearch.trim().length > 0 && returnVols.length > 0) {
    // Vérifier que le texte ne correspond pas à un vol sélectionné
...
}, [returnVolSearch, returnVols, selectedReturnFlight, selectionLocked]);
useEffect(() => {
  // ✅ NE PAS RÉAFFICHER PENDANT LE VERROUILLAGE
  if (selectionLocked) {
    return;
  }
  
  // Ne réafficher que si la recherche n'est pas vide et qu'il y a des résultats
  if (volSearch.trim().length > 0 && vols.length > 0) {
    // NE PAS réafficher si le texte correspond à un vol sélectionné
...
}, [volSearch, vols, selectedFlight, selectionLocked]);
  const handleReturnFlightSelect = (vol) => {
  if (!vol || selectionLocked) return; // Empêcher les doubles clics avec le même verrouillage

  // ACTIVER LE VERROUILLAGE
  setSelectionLocked(true);

  // 1. Mettre à jour le texte de recherche
  setReturnVolSearch(formatFlightDisplay(vol));

  // 2. FERMER TOUTES LES SUGGESTIONS
  setShowReturnVolSuggestions(false);
  setShowVolSuggestions(false);
  setShowCompanySuggestions(false);
  setShowStartDestinationSuggestions(false);
  ...
  return (
    <div className="space-y-6 p-4 bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl border-2 border-green-200">
      ...
      {/* ✅ Section Compagnie CORRIGÉE - avec hauteur max et scroll */}
      <div className="bg-white w-full p-4 rounded-lg border border-gray-200">
        ...
        {/* ✅ Suggestions de compagnies avec hauteur limitée et scroll */}
        ...
        <div className="relative">
          <input
            ...
            onChange={(e) => {
              ...
              // ✅ Ne pas désactiver le filtre compagnie ici
            }}
            onFocus={() => {
        // ✅ NE RÉOUVRIR QUE SI DÉVERROUILLÉ
        ...
        {loadingClasses ? (
          ...
          <div className="space-y-3">
            <select
              ...
              className="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-green-500 focus:ring-green-200 transition-colors bg-white"
              required
            >
              <option value="">-- Choisissez une classe --</option>
              {classes.map(cls => (
                <option key={cls.id} value={cls.id}>
                  {cls.className} - Multiplicateur: x{cls.priceMultiplier}
                </option>
              ))}
            </select>
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-3 mt-2">
                      <span className="text-xs bg-blue-100 text-blue-800 px-2 py-1 rounded-full">
                        Multiplicateur: x{selectedClass.priceMultiplier}
                      </span>
                    </div>
                  </div>
                </div>
...
// Le reste du composant CreateReservationCampaign reste identique...
// (Je n'inclus pas la suite pour éviter la répétition, mais elle reste la même que dans la version précédente)
const CreateReservationCampaign = () => {
 ...
 console.log('campaignSelection',campaigns)
    return (
